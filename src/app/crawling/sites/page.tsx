		...
		<div className="space-y-6 p-6">
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-muted-foreground mt-2">
						크롤링 대상 사이트를 관리하고 모니터링할 수 있습니다. </p>
				</div>
			</div>
					...
					<div className="flex justify-between items-center">
						...
						<Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
							<DialogTrigger asChild>
								<Button>
									<Plus className="mr-2 h-4 w-4"/> 사이트 추가
								</Button>
							</DialogTrigger>
							<DialogContent className="sm:max-w-[600px]">
								...
								<div className="space-y-4 py-4">
									<div className="space-y-2">
										...
										<Input id="name" name="name" placeholder="예: 네이버 카페" value={newSite.name} onChange={handleNewSiteChange}/>
									</div>
									<div className="space-y-2">
										<label htmlFor="url">URL</label>
										<Input id="url" name="url" placeholder="https://" value={newSite.url} onChange={handleNewSiteChange}/>
									</div>
									<div className="space-y-2">
										<label htmlFor="xpath">XPath</label>
										<Input id="xpath" name="xpath" placeholder="게시물 목록 XPath" value={newSite.xpath} onChange={handleNewSiteChange}/>
									</div>
									<div className="space-y-2">
										<label htmlFor="assistantName">Assistant 이름</label>
										<Input id="assistantName" name="assistantName" placeholder="크롤링 담당 Assistant 이름" value={newSite.assistantName} onChange={handleNewSiteChange}/>
									</div>
									<div className="space-y-2">
										<label htmlFor="interval">크롤링 주기 (분)</label>
										<Input id="interval" name="interval" type="number" min="1" placeholder="5" value={newSite.interval} onChange={handleNewSiteChange}/>
									</div>
									<div className="flex items-center space-x-2">
										<Switch id="isActive" name="isActive" checked={newSite.isActive} onCheckedChange={(checked) =>
											setNewSite((prev) => ({
												...prev,
												isActive: checked
											}))
				...
				<CardContent>
					<div className="flex justify-between items-center mb-4">
						<div className="flex w-full max-w-sm items-center space-x-2">
							<Input placeholder="사이트명 또는 URL로 검색" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-md"/>
									...
									siteData
										...
										.map((site) => (
											<TableRow key={site.id}>
												<TableCell>
													<Badge variant={site.isActive ? "success" : "secondary"} className="w-20 justify-center">
												...
												<TableCell className="text-right">
													<div className="flex justify-end space-x-2">
														...
														<Button variant="outline" size="icon" className="text-destructive" onClick={() => handleDeleteSite(site.id)}>
			...
			<Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
				<DialogContent className="sm:max-w-[600px]">
					...
					</DialogHeader> {editSite && (
					<div className="space-y-4 py-4">
						<div className="space-y-2">
							<label htmlFor="edit-name">사이트명</label>
							<Input id="edit-name" name="name" placeholder="예: 네이버 카페" value={editSite.name} onChange={handleEditSiteChange}/>
						</div>
						<div className="space-y-2">
							<label htmlFor="edit-url">URL</label>
							<Input id="edit-url" name="url" placeholder="https://" value={editSite.url} onChange={handleEditSiteChange}/>
						</div>
						<div className="space-y-2">
							<label htmlFor="edit-xpath">XPath</label>
							<Input id="edit-xpath" name="xpath" placeholder="게시물 목록 XPath" value={editSite.xpath} onChange={handleEditSiteChange}/>
						</div>
						<div className="space-y-2">
							<label htmlFor="edit-assistantName">Assistant 이름</label>
							<Input id="edit-assistantName" name="assistantName" placeholder="크롤링 담당 Assistant 이름" value={editSite.assistantName} onChange={handleEditSiteChange}/>
						</div>
						<div className="space-y-2">
							<label htmlFor="edit-interval">크롤링 주기 (분)</label>
							<Input id="edit-interval" name="interval" type="number" min="1" placeholder="5" value={editSite.interval} onChange={handleEditSiteChange}/>
						</div>
						<div className="flex items-center space-x-2">
							<Switch id="edit-isActive" name="isActive" checked={editSite.isActive} onCheckedChange={(checked) =>
